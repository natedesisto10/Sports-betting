...
def get_elo():
    ...
    elo_df = elo_df.drop(columns = ['season','neutral' ,'playoff','score1', 'score2', 'qb1', 'qb2', 'elo1_post', 'elo2_post',
                                    'qbelo1_pre', 'qbelo2_pre', 'qb1_game_value', 'qb2_game_value', 'qb1_value_post',
                                    'qb2_value_post', 'qbelo1_post', 'qbelo2_post',])
    elo_df.date = pd.to_datetime(elo_df.date)
    elo_df = elo_df[elo_df.date < '01-05-2021']
    #  'elo_prob1', 'elo_prob2',  'qb1_adj', 'qb2_adj', 'qbelo_prob1', 'qbelo_prob2',   
...
def display2(y_pred,X_test):
    for g in range(len(y_pred)):
        win_prob = round(y_pred[g],2)
        away_team = X_test.reset_index().drop(columns = 'index').loc[g,'away_name']
        home_team = X_test.reset_index().drop(columns = 'index').loc[g,'home_name']
        if win_prob <= .45:
            print(f'The {home_team} beats the {away_team}.')
        elif win_prob >= .55:
            print(f'The {away_team} beats the {home_team}.') 
        else:
            print(f'The {home_team} and {away_team} game will be interesting.')
